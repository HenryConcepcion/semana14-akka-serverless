...
/**
 * Actor trabajador que procesa tareas.
 */
    ...
    public Receive createReceive() {
        return receiveBuilder()
            .match(TaskMessage.class, message -> {
                System.out.println("Worker " + self().path().name()
                        + " recibió: " + message.getText());

                if (message.shouldSimulateFailure()) {
                    System.out.println("Worker " + self().path().name()
                            + " está simulando un fallo...");
                    throw new RuntimeException("Fallo intencional del Worker");
                }

                String resultado = "Procesado correctamente: "
