...
public class SupervisorActor extends AbstractLoggingActor {
    ...
    public void preStart() {
        ...
        worker1 = getContext().actorOf(
                WorkerActor.props(),
                "worker1"
        );

        worker2 = getContext().actorOf(
                WorkerActor.props(),
                "worker2"
        );

        worker3 = getContext().actorOf(
                WorkerActor.props(),
                "worker3"
        );
    ...
    public Receive createReceive() {
        ...
        return receiveBuilder()
                ...
                .match(TaskMessage.class, message -> {
                    ...
                    System.out.println(
                            "Supervisor recibió una nueva tarea."
                    );

                    // Enviar las tareas a diferentes workers
                    worker1.tell(message, self());

                    worker2.tell(
                            new TaskMessage(
                                    message.getText() + " - tarea 2",
                                    false
                            ),
                            self()
                    );
